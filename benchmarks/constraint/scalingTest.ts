...
import { benchmark, formatMs } from './constraintBench.ts';
...
for (const targetDeps of DEP_COUNTS) {
    ...
    console.log(
        `─── ${relationships.length} relationships (${relsPerTask} per task) ───`,
    );
    ...
    const r4 = benchmark('calculateCascadeUpdates', () => {
        calculateCascadeUpdates(
            firstTaskId,
            tasks[firstTaskId]._bar.x + 10,
            context,
        );
    });

    console.log(
        `  resolveConstraints:        mean=${formatMs(r1.mean).padEnd(10)} p95=${formatMs(r1.p95)}`,
    );
    console.log(
        `  getMinXFromPredecessors:   mean=${formatMs(r2.mean).padEnd(10)} p95=${formatMs(r2.p95)}`,
    );
    console.log(
        `  getMaxEndFromDownstream:   mean=${formatMs(r3.mean).padEnd(10)} p95=${formatMs(r3.p95)}`,
    );
    console.log(
        `  calculateCascadeUpdates:   mean=${formatMs(r4.mean).padEnd(10)} p95=${formatMs(r4.p95)}`,
    );
    console.log('');
...
console.log('');
console.log(
    'Relationships     | resolveAll | getMinX    | getMaxDown | cascade',
);
console.log('─'.repeat(70));
...
for (const r of results) {
    ...
    const r1x =
        (r.resolveConstraints / baseline.resolveConstraints).toFixed(1) + 'x';
    const r2x =
        (r.getMinXFromPredecessors / baseline.getMinXFromPredecessors).toFixed(
            1,
        ) + 'x';
    const r3x =
        (r.getMaxEndFromDownstream / baseline.getMaxEndFromDownstream).toFixed(
            1,
        ) + 'x';
    const r4x =
        (r.calculateCascadeUpdates / baseline.calculateCascadeUpdates).toFixed(
            1,
        ) + 'x';

    console.log(
        `${String(r.relationships).padEnd(6)} (${scale.padEnd(3)}) | ${formatMs(r.resolveConstraints).padEnd(7)} ${r1x.padEnd(5)} | ${formatMs(r.getMinXFromPredecessors).padEnd(7)} ${r2x.padEnd(5)} | ${formatMs(r.getMaxEndFromDownstream).padEnd(7)} ${r3x.padEnd(5)} | ${formatMs(r.calculateCascadeUpdates).padEnd(7)} ${r4x}`,
    );
}
