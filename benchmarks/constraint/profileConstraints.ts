...
console.log(
    `Iterations: ${ITERATIONS}, Mode: ${USE_INDEX ? 'INDEXED' : 'BASELINE'}`,
);
console.log('');
...
function instrumentedResolveConstraints(taskId, proposedX, proposedWidth, ctx) {
    ...
    t0 = performance.now();
    const cascadeUpdates = blocked
        ? new Map()
        : calculateCascadeUpdates(taskId, constrainedX, ctx);
    t1 = performance.now();
...
for (let i = 0; i < ITERATIONS; i++) {
    instrumentedResolveConstraints(
        taskId,
        task._bar.x + 10,
        task._bar.width,
        context,
...
const breakdown = [
    ...
    {
        name: 'getMaxXFromLockedSuccessors',
        time: timings.getMaxXFromLockedSuccessors,
    },
...
for (const item of breakdown) {
    ...
    console.log(
        `${item.name.padEnd(28)} ${avgMs.toFixed(3).padStart(7)}ms  ${pct.toFixed(1).padStart(5)}%  ${bar}`,
    );
}

console.log('─'.repeat(70));
console.log(
    `${'TOTAL'.padEnd(28)} ${(total / ITERATIONS).toFixed(3).padStart(7)}ms  100.0%`,
);
console.log('');
...
console.log(
    '├── lockCheck                     ' +
        ((timings.lockCheck / total) * 100).toFixed(1) +
        '%',
);
console.log(
    '├── absoluteConstraints           ' +
        ((timings.absoluteConstraints / total) * 100).toFixed(1) +
        '%',
);
console.log(
    '├── getMinXFromPredecessors       ' +
        ((timings.getMinXFromPredecessors / total) * 100).toFixed(1) +
        '%',
);
console.log('│   └── for(rel of relationships) O(n) scan');
console.log(
    '├── getMaxXFromPredecessors       ' +
        ((timings.getMaxXFromPredecessors / total) * 100).toFixed(1) +
        '%',
);
console.log('│   └── for(rel of relationships) O(n) scan');
console.log(
    '├── getMaxEndFromDownstream       ' +
        ((timings.getMaxEndFromDownstream / total) * 100).toFixed(1) +
        '% ◀◀◀ BOTTLENECK',
);
...
console.log(
    '├── getMaxXFromLockedSuccessors   ' +
        ((timings.getMaxXFromLockedSuccessors / total) * 100).toFixed(1) +
        '%',
);
console.log('│   └── for(rel of relationships) O(n) scan');
console.log(
    '├── clampAndCheck                 ' +
        ((timings.clampAndCheck / total) * 100).toFixed(1) +
        '%',
);
console.log(
    '└── calculateCascadeUpdates       ' +
        ((timings.calculateCascadeUpdates / total) * 100).toFixed(1) +
        '%',
);
