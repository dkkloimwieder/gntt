...
interface SubtaskConfig {
    totalTasks: number;
    parentTaskRatio: number;
    minSubtasks: number;
    maxSubtasks: number;
    subtaskLayouts: ('sequential' | 'parallel' | 'mixed')[];
    startDate: string;
    taskDurationDays: { min: number; max: number };
    subtaskDurationDays: { min: number; max: number };
    dependencyChance: number;
    seed: number;
}

interface GeneratedSubtaskTask {
    id: string;
    name: string;
    resource: string;
    color: string;
...
interface SubtaskDemoResult {
    tasks: GeneratedSubtaskTask[];
...
function pick<T>(random: () => number, array: T[]): T {
...
export function generateSubtaskDemo(config: Partial<SubtaskConfig> = {}): SubtaskDemoResult {
    ...
    const tasks: GeneratedSubtaskTask[] = [];
    ...
    for (let i = 0; i < targetParents; i++) {
        ...
        const parentTask: GeneratedSubtaskTask = {
            id: parentId,
        ...
        for (let j = 0; j < numSubtasks; j++) {
            ...
            const subtask: GeneratedSubtaskTask = {
                id: `task-${taskId++}`,
