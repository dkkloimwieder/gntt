...
export function parse_duration(duration: string): DurationResult | undefined {
...
export function get_date_values(
    date: Date,
...
export function to_string(date: Date, with_time = false): string {
    if (!(date instanceof Date)) {
        throw new TypeError('Invalid argument type');
    }
    const [year, month, day, hour, min, sec, ms] = get_date_values(date);
    const vals = [
        padStart(year + '', 4, '0'),
...
export function today(): Date {
    const vals = get_date_values(new Date()).slice(0, 3) as [
        number,
        number,
        number,
    ];
...
export function add(date: Date, qty: number | string, scale: TimeScale): Date {
...
export function start_of(date: Date, scale: TimeScale): Date {
...
export function clone(date: Date): Date {
    const vals = get_date_values(date);
    return new Date(
        vals[0],
        vals[1],
        vals[2],
...
export function convert_scales(period: string, to_scale: TimeScale): number {
...
export function get_days_in_month(date: Date): number {
...
export function get_days_in_year(date: Date): number {
    return date.getFullYear() % 4 ? 365 : 366;
}

// camelCase aliases (kept for API compatibility)
export const parseDuration = parse_duration;
export const toString = to_string;
export const startOf = start_of;
export const getDateValues = get_date_values;
export const convertScales = convert_scales;
export const getDaysInMonth = get_days_in_month;
export const getDaysInYear = get_days_in_year;
