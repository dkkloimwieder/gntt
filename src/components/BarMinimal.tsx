 ...
 */
export function BarMinimal(props: BarMinimalProps): JSX.Element {
    // Handle both signal and direct value
    const getTask = (): ProcessedTask | undefined =>
        typeof props.task === 'function' ? props.task() : props.task;

    // Single memo batching ALL reads
    const t = createMemo<ComputedBarData>(() => {
        const task = getTask();
        const progress = task?.progress ?? 0;
        const width = task?._bar?.width ?? 100;
        return {
            color: (task?.color as string) ?? '#3b82f6',
            progress,
            name: task?.name ?? '',
