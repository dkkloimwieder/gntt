...
console.log(
    `Grid: ${TOTAL_COLS}x${TOTAL_ROWS} = ${calendarData.tasks.length} tasks`,
);
...
const maxX = Math.max(
    ...Object.values(initialTasks).map((t) => t._bar.x + t._bar.width),
);
...
function TestBar(props) {
    ...
    const bg = () => {
        ...
        const lockedOverlay = data.locked
    ...
    const runStressTest = (mode) => {
        ...
        const tick = () => {
            if (controller.abort || performance.now() - startTime > duration) {
                setRunning(false);
                setTestMode(null);
                console.log(`${mode} test complete:`, {
                    fps: fps(),
                    worst: worstFrame(),
                    avg: avgFrame(),
                });
            ...
            const scrollArea =
                containerRef?.querySelector('.gantt-scroll-area') || scrollRef;
            if (scrollArea) {
                // Vertical scroll
    ...
    return (
        <GanttEventsProvider>
            ...
            >
                ...
                >
                    <h2 style={{ margin: 0 }}>
                        2D Scroll Test ({TOTAL_COLS}x{TOTAL_ROWS})
                    </h2>
                    <div
                        style={{
                            display: 'flex',
                    ...
                    >
                        ...
                        <span>
                            FPS: <b style={{ color: fpsColor() }}>{fps()}</b>
                        </span>
                        <span>
                            Worst: <b>{worstFrame()}ms</b>
                        </span>
                        <span>
                            Avg: <b>{avgFrame()}ms</b>
                        </span>
